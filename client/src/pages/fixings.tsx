...
export default function Fixings() {
  ...
  // ⬇️ (FIX) Tout le reste doit rester **à l'intérieur** du composant — on a retiré le `}` qui fermait ici.
  ...
  const vessels = (vesselsData as any)?.data || [];
  ...
  useEffect(() => {
    ...
    try {
      ...
      const period = sp.get("period"); // présent si besoin
      if (price) {
        prefillOnceRef.current = true;
        setOpen(true);
        setForm((f: any) => ({ ...f, priceUsd: price, grade: g || f.grade }));
        // Nettoie la query pour éviter la réouverture au refresh :
  ...
  const mutate = useMutation({
    mutationFn: async (payload: any) => {
      const res = await fetch("/api/fixings", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
      if (!res.ok) throw new Error("Failed to save fixing");
      return res.json();
    },
    onSuccess: () => {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape") setShowPriceMenu(false);
    };
        ...
        <main className="flex-1 overflow-y-auto p-6">
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Button onClick={() => setOpen(true)} className="bg-trading-blue">
                Add Fixing
              </Button>
            </div>
                  ...
                  <table className="min-w-full text-sm">
                    <thead className="text-gray-300">
                      <tr className="text-left">
                        ...
                        <th className="py-2 px-3">Price (USD/MT)</th>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-gray-900 border border-gray-700 rounded-lg p-4 w-[680px]">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setShowPriceMenu(false); // reset si on change de grade
                  }}
              ...
              {/* Price with dropdown of forwards */}
              <div className="relative">
                <Label className="text-sm">Price (USD/MT)</Label>
                <Input
                  value={form.priceUsd}
                  onChange={(e) => setForm({ ...form, priceUsd: e.target.value })}
                  onFocus={() => setShowPriceMenu(true)}
                  onClick={() => setShowPriceMenu(true)}
                  ...
                  <div className="absolute z-50 mt-1 w-72 max-h-64 overflow-auto bg-gray-900 border border-gray-700 rounded-lg shadow-lg p-2">
                    {getForwards(form.grade).length === 0 ? (
                      <div className="text-sm text-gray-400 p-2">
                        Select a grade to see forward prices.
                      </div>
                    ) : (
                      getForwards(form.grade).map((p: any, idx: number) => (
                        <button
                          ...
                          onClick={() => {
                            setForm({ ...form, priceUsd: String(p.price) });
              ...
              <div className="col-span-2">
                ...
                <select
                  ...
                  className="bg-gray-800 border border-gray-700 rounded px-2 py-2 w-full"
                >
                  <option value="">-- select --</option>
                  {vessels.map((v: any) => (
            ...
            <div className="flex justify-end gap-2 mt-4">
              ...
              <Button className="bg-trading-blue" onClick={() => mutate.mutate(form)}>
                Save
              </Button>
            </div>
          </div>
